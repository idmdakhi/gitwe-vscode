...
export async function publishBranchCommand(
  ...
  try {
    ...
    const remoteRef = await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: `Gitwe: publishing ${target}…` },
      () => engine.publish(resolved),
    );
    void vscode.window.showInformationMessage(`Gitwe: published ${target} to ${remoteRef}.`);
  } catch (error) {
