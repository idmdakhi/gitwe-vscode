...
async function withRepo<T>(
  ...
  if (!repo) {
    void vscode.window.showWarningMessage(
      "gitwe: open a workspace folder first.",
    );
    return undefined;
  }
  try {
...
async function reportError(err: unknown): Promise<void> {
  ...
  void vscode.window.showErrorMessage(
    `gitwe: ${err instanceof Error ? err.message : String(err)}`,
  );
}
...
function branchQuickPickItems(
  types: GitweTypeDefinition[],
): (vscode.QuickPickItem & { type: GitweTypeDefinition })[] {
  return types.map((t) => {
    const target = toArray(t.target);
    const aliases = toArray(t.aliases);
    return {
      type: t,
      label: `$(git-branch) ${t.name}`,
      description: `${t.prefix}<name> → ${target || "?"}`,
      detail: `base: ${t.base}${aliases.length ? `  ·  aliases: ${toArray(aliases)}` : ""}`,
    };
  });
}
...
export function registerCommands(
  ...
  subscriptions.push(
    vscode.commands.registerCommand("gitwe.init", async () => {
      const preset = await vscode.window.showQuickPick(
        [
          {
            label: "classic",
            description: "git-flow: develop + feature/release/hotfix",
          },
          {
            label: "github",
            description: "GitHub Flow: main + short-lived topic branches",
          },
          {
            label: "gitlab",
            description: "GitLab Flow: main + environment branches",
          },
        ],
      ...
      void vscode.window.showInformationMessage(
        `gitwe: initialized with the "${preset.label}" preset.`,
      );
    }),
    ...
    vscode.commands.registerCommand("gitwe.start", async () => {
      ...
      const { types } = await repo
        .types()
        .catch(() => ({ types: [] as GitweTypeDefinition[] }));
      if (!types.length) {
        void vscode.window.showWarningMessage(
          "gitwe: no topic types configured. Run `gitwe init` first.",
        );
        return;
      }
      const picked = await vscode.window.showQuickPick(
        branchQuickPickItems(types),
        {
      ...
      const name = await vscode.window.showInputBox({
        ...
        validateInput: (v) =>
          v.trim() ? undefined : "a short name is required",
      });
      if (!name) return;
      await withRepo(cmdCtx, (r) =>
        r.start(picked.type.name, name.trim(), undefined),
      );
      void vscode.window.showInformationMessage(
        `gitwe: started ${picked.type.prefix}${name.trim()}`,
      );
    }),

    vscode.commands.registerCommand("gitwe.finishCurrent", () =>
      finishBranch(cmdCtx, undefined),
    ),
    vscode.commands.registerCommand(
      "gitwe.finishBranchByName",
      (branch: string) => finishBranch(cmdCtx, branch),
    ),
    ...
    vscode.commands.registerCommand("gitwe.updateCurrent", async () => {
      ...
      await withRepo(cmdCtx, (repo) =>
        repo.update(undefined, rebase ? ["--rebase"] : []),
      );
    }),
    ...
    vscode.commands.registerCommand("gitwe.deleteCurrent", () =>
      deleteBranch(cmdCtx, undefined),
    ),
    ...
    vscode.commands.registerCommand("gitwe.checkout", async () => {
      ...
      if (!branches.length) {
        void vscode.window.showInformationMessage(
          "gitwe: no topic branches to check out.",
        );
        return;
      }
      const picked = await vscode.window.showQuickPick(
    ...
    vscode.commands.registerCommand("gitwe.tag", async () => {
      ...
      const push = await vscode.window.showQuickPick(["Yes", "No"], {
        title: "push the tag?",
      });
      const extraArgs = [
        ...(message ? ["-m", message] : []),
    ...
    vscode.commands.registerCommand("gitwe.abort", async () => {
      if (
        !(await confirmDestructive(
          "Abort the in-progress gitwe finish and roll back all changes it made?",
        ))
      )
        return;
    ...
    vscode.commands.registerCommand("gitwe.doctor", () =>
      runDoctor(cmdCtx, false),
    ),
    vscode.commands.registerCommand("gitwe.doctorFix", () =>
      runDoctor(cmdCtx, true),
    ),
    ...
    vscode.commands.registerCommand("gitwe.validate", async () => {
      ...
      if (result.valid) {
        void vscode.window.showInformationMessage(
          "gitwe: workflow definition is valid.",
        );
      } else {
        const doc = await vscode.workspace.openTextDocument({
    ...
    vscode.commands.registerCommand("gitwe.refreshBranches", () =>
      cmdCtx.refreshAll(),
    ),

    // ---- Tree view item actions -------------------------------------------------
    vscode.commands.registerCommand(
      "gitwe.branch.checkout",
    ...
    vscode.commands.registerCommand(
      "gitwe.branch.finish",
      async (node: BranchNode) => finishBranch(cmdCtx, node.branch.branch),
    ),
    vscode.commands.registerCommand(
      "gitwe.branch.rename",
      async (node: BranchNode) => {
  ...
  async function deleteBranch(
    ...
    if (
      !(await confirmDestructive(
        `Delete ${label}? This cannot be undone from within gitwe.`,
      ))
    )
      return;
    const alsoRemote = await vscode.window.showQuickPick(
    ...
    await withRepo(ctx, (repo) =>
      repo.deleteBranch(
        branch,
        alsoRemote === "Local + remote" ? ["--remote"] : [],
      ),
    );
  }

  async function pickMergeOrRebase(
    title: string,
  ): Promise<boolean | undefined> {
    const choice = await vscode.window.showQuickPick(["Merge", "Rebase"], {
      title,
    });
  ...
  async function runDoctor(ctx: CommandContext, fix: boolean): Promise<void> {
    ...
    try {
      ...
      if (errors > 0)
        void vscode.window
          .showErrorMessage(summary, "Open Dashboard")
          .then(
            (c) => c && vscode.commands.executeCommand("gitwe.openDashboard"),
          );
